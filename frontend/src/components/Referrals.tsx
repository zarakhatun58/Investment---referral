  ...
  depth = 0
    ...
    return (
      <div key={node._id} className="relative">
        <div
          className={`flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer ${depth === 0 ? 'bg-emerald-50 border border-emerald-100' : 'bg-white'
            }`}
        ...
        >
          ...
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${depth === 0
                ? 'bg-gradient-to-br from-emerald-500 to-cyan-500'
                : 'bg-gradient-to-br ' + getNodeGradient(depth)
              }`}
          >
            {node.fullName?.charAt(0) || "U"}
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="font-medium text-slate-800 truncate">{node.fullName || "Unknown User"}</p>
              {depth === 0 && (
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-slate-400">{node.createdAt
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-slate-100">
            {directReferrals.map((referral) => (
              <div key={referral._id} className="flex items-center gap-4 p-4 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-800 truncate">{referral.fullName}</p>
                  <p className="text-sm text-slate-500 truncate">{referral.email}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-slate-600">Code: {referral.referralCode}</p>
                  <p className="text-xs text-slate-400">{formatDate(referral.createdAt)}</p>
                </div>
              </div>
            ))}
          </div>
